...
namespace EntityFramework_Linq
{
	public partial class frm_CourseSelection : Form
	{
		...
		private void btn_Load_Click(object sender, EventArgs e)
		{
			...
			this.dgv_SelectedCourse.Columns["No"].HeaderText = "编号";
			...
			this.dgv_SelectedCourse.Columns["State"].Visible = false;
			this.dgv_SelectedCourse.Columns["CanBeRemoved"].Visible = false;
			this.dgv_SelectedCourse.Columns[3].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
			this.lbl_CreditSum.Text =
				$"共{this.SelectedCourseInfos.SumCredit()}学分";
		}
		/// <summary>
		/// 点击添加按钮；
		/// </summary>
		...
		private void btn_Add_Click(object sender, EventArgs e)
		{
			...
			var selectedCourse = CourseService.Select(currentCourse);
		...
		private void btn_Remove_Click(object sender, EventArgs e)
		{
			...
			if (!currentSelectedCourse.CanBeRemoved)
				return;
			var course = CourseService.Reject(currentSelectedCourse);
			this.CourseInfos.Add(course);
			this.SelectedCourseInfos.Remove(currentSelectedCourse);
			this.lbl_CreditSum.Text =
				$"共{this.SelectedCourseInfos.SumCredit()}学分";
		}
		/// <summary>
		...
		{
			int rowAffected =
				this.SelectedCourseRepository.Save(this.SelectedCourseInfos.ToList(), this.StudentNo);
